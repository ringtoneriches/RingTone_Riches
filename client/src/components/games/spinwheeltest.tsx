...
const SpinWheel: React.FC<SpinWheelProps> = ({
  ...
  // Transform admin wheel config to component format
  const segments = (wheelConfig?.segments || []).map((seg) => {
  ...
  useEffect(() => {
    ...
    segments.forEach((segment, index) => {
      ...
      // Timeout fallback - resolve after 1.5 seconds even if not loaded
      const timeout = setTimeout(() => {
        if (!imagesArray[index]) {
          imagesArray[index] = new Image();
          loadedCount++;
          scheduleUpdate();
          if (loadedCount === segments.length) setAllImagesLoaded(true);
        }
      }, 1500);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen relative overflow-hidden z-10">
      ...
      >
        <source
          src="https://res.cloudinary.com/dziy5sjas/video/upload/v1761231208/backgroundwheelvideo_cjhrwq.mp4"
          type="video/mp4"
        />
      </video>
                    ...
                    <td className="px-1 sm:px-2 md:px-3 py-2 sm:py-3 text-right rounded-r-lg">
                      <span className="text-green-400 font-bold text-xs sm:text-sm whitespace-nowrap">
                        ...
                        item.prize.amount > 0
                          ...
                          : typeof item.prize.amount === "string" &&
