...
>([]);
  ...
  useEffect(() => {
    // Just generate random images for display - winner determined server-side
  ...
  if (percent >= AUTO_CLEAR_THRESHOLD && !revealed && !hasCompletedRef.current) {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        // Show matching images if they won
                    ...
                    >
                      ...
                      <td className="px-1 sm:px-2 md:px-3 py-2 sm:py-3 text-right font-bold rounded-r-lg">
                        {item.prize.type === "cash" ? (
                          <span className="text-[#FACC15] flex items-center justify-end gap-0.5 sm:gap-1">
                            <span className="text-sm sm:text-lg">💰</span>
                            <span className="text-xs sm:text-sm md:text-base">£{item.prize.value}</span>
                          </span>
                        ) : item.prize.type === "points" ? (
                          <span className="text-[#F59E0B] flex items-center justify-end gap-0.5 sm:gap-1">
                            <span className="text-sm sm:text-lg">⭐</span>
                            <span className="text-xs sm:text-sm md:text-base whitespace-nowrap">{item.prize.value}<span className="hidden sm:inline"> pts</span></span>
                          </span>
                        ) : (
                          <span className="text-gray-400 text-xs sm:text-sm">{item.prize.value}</span>
                        )}
                      </td>
                    </tr>
                  ))}
