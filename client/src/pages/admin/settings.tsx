...
export default function AdminSettings() {
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async (data: { commissionRate: string; minimumTopUp: string }) => {
  ...
  const changeUsernameMutation = useMutation({
    mutationFn: async (data: { newUsername: string }) => {
      return await apiRequest("POST", "/api/admin/change-username", data);
    },
    onSuccess: () => {
      ...
      <div className="space-y-4 md:space-y-6">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm md:text-base text-muted-foreground mt-1">Configure your competition platform</p>
          </div>
        </div>
