...
export default function Wallet() {
  ...
  const withdrawalRequestMutation = useMutation({
    mutationFn: async (data: { amount: string; accountName: string; accountNumber: string; sortCode: string }) => {
      const response = await apiRequest("/api/withdrawal-requests", "POST", data);
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: "Success!",
        description: "Withdrawal request submitted. We'll process it manually within 2-3 business days.",
      });
      setWithdrawalDialogOpen(false);
      setWithdrawalForm({ amount: "", accountName: "", accountNumber: "", sortCode: "" });
      queryClient.invalidateQueries({ queryKey: ["/api/withdrawal-requests/me"] });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-zinc-900 to-black text-white">
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            ...
            <TabsContent value="wallet" className="space-y-6 pt-12 relative z-0" data-testid="content-wallet">
              ...
              {withdrawalRequests.length > 0 && (
                <Card className="bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-800 border-yellow-500/30 shadow-xl shadow-yellow-500/10">
                  <CardHeader className="border-b border-yellow-500/20">
                    <CardTitle className="text-2xl text-yellow-400">Withdrawal Requests</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4">
                    <div className="space-y-3">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-400 space-y-1">
                              ...
                              <p>Sort Code: {request.sortCode} | Account: ****{request.accountNumber.slice(-4)}</p>
                              <p>Requested: {format(new Date(request.createdAt), "dd MMM yyyy 'at' HH:mm")}</p>
                              {request.adminNotes && (
                                <p className="text-yellow-400 mt-2">Note: {request.adminNotes}</p>
                              )}
                            </div>
                          </div>
                        );
            ...
            <TabsContent value="address" className="space-y-6 pt-12 relative z-0" data-testid="content-address">
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-6">
                <p className="text-sm text-blue-400 font-medium">
                  ℹ️ Prize winners will be contacted directly to arrange delivery. Save your address here for faster processing.
                </p>
              </div>
              <Card className="bg-zinc-900 border-yellow-500/30 shadow-xl shadow-yellow-500/10">
                <CardHeader className="border-b border-yellow-500/20">
      ...
      <Dialog open={withdrawalDialogOpen} onOpenChange={setWithdrawalDialogOpen}>
        <DialogContent className="bg-zinc-900 border-yellow-500/30" data-testid="dialog-withdrawal">
          <DialogHeader>
            <DialogTitle className="text-yellow-400">Request Withdrawal</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Amount (£)</Label>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setWithdrawalForm({ ...withdrawalForm, sortCode: e.target.value.replace(/\D/g, '') })}
                className="bg-black/50 border-yellow-500/30 text-white"
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-sm text-gray-300">
              ...
              <p>Withdrawal requests are processed manually within 2-3 business days. You'll receive your funds via bank transfer.</p>
            </div>
          </div>
          <DialogFooter>
            <Button
              onClick={() => withdrawalRequestMutation.mutate(withdrawalForm)}
              disabled={withdrawalRequestMutation.isPending || !withdrawalForm.amount || !withdrawalForm.accountName || !withdrawalForm.accountNumber || !withdrawalForm.sortCode}
              className="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black hover:from-yellow-500 hover:to-yellow-400"
              data-testid="button-submit-withdrawal"
            >
              {withdrawalRequestMutation.isPending ? "Submitting..." : "Submit Request"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
