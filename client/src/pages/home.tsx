          ...
          <div className="flex flex-wrap justify-center gap-2 md:gap-3">
            {[
              ...
              <button
                ...
                className={`relative px-4 md:px-8 py-2.5 md:py-3 rounded-lg font-black text-xs md:text-sm uppercase tracking-wide transition-all overflow-hidden group ${
              ...
              >
                ...
                <span className={`relative z-10 flex items-center gap-1.5 md:gap-2 ${
                  activeFilter === filter.id ? "text-slate-900" : ""
                }`}>
                  <filter.icon className="w-3 h-3 md:w-4 md:h-4" />
                  <span className="hidden sm:inline">{filter.label}</span>
                  <span className="sm:hidden">{filter.id.toUpperCase()}</span>
                </span>
              </button>
            ))}
          </div>
